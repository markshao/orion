...
func TestPushBranch(t *testing.T) {
    ...
    // Push main branch first (required before pushing other branches)
    ...
    cmd = exec.Command("git", "ls-remote", remoteDir, "refs/heads/feature/test")
    output, err := cmd.CombinedOutput()
    if err != nil {
        t.Errorf("feature/test branch not found in remote: %v, output: %s", err, output)
    }
    if len(bytes.TrimSpace(output)) == 0 {
        t.Errorf("feature/test branch not pushed to remote")
    }
}
