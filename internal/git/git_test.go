...
func setupTestRepoWithRemote(t *testing.T) (localPath, remotePath string, cleanup func()) {
    ...
    // Add remote
    _ = exec.Command("git", "-C", localDir, "remote", "add", "origin", remoteDir).Run()

    // Push main branch
    _ = exec.Command("git", "-C", localDir, "push", "-u", "origin", "main").Run()
...
func TestPushBranch(t *testing.T) {
    ...
    if err := CreateBranch(localPath, "feature/test-push", "main"); err != nil {
        t.Fatalf("CreateBranch failed: %v", err)
    }
    ...
    _ = exec.Command("git", "-C", localPath, "add", ".").Run()
    _ = exec.Command("git", "-C", localPath, "commit", "-m", "Add feature").Run()

    // Push the branch
    if err := PushBranch(localPath, "feature/test-push"); err != nil {
    ...
    if !strings.Contains(string(output), "feature/test-push") {
        t.Errorf("branch feature/test-push not found in remote. Output: %s", string(output))
    }
}
...
func TestPushBranchAlreadyExists(t *testing.T) {
    ...
    if err := CreateBranch(localPath, "feature/existing", "main"); err != nil {
        t.Fatalf("CreateBranch failed: %v", err)
    }

    _ = exec.Command("git", "-C", localPath, "add", ".").Run()
    _ = exec.Command("git", "-C", localPath, "commit", "-m", "Initial").Run()

    // First push
    if err := PushBranch(localPath, "feature/existing"); err != nil {
        t.Fatalf("First PushBranch failed: %v", err)
    }

    // Verify first push succeeded
    output, err := exec.Command("git", "-C", remotePath, "branch", "-a").CombinedOutput()
    if err != nil {
        t.Fatalf("failed to list remote branches: %v", err)
    }
    if !strings.Contains(string(output), "feature/existing") {
        t.Fatalf("branch feature/existing not found in remote after first push")
    }

    // Make another commit
    newFile := filepath.Join(localPath, "update.txt")
    if err := os.WriteFile(newFile, []byte("update content"), 0644); err != nil {
        t.Fatalf("failed to write file: %v", err)
    }

    _ = exec.Command("git", "-C", localPath, "add", ".").Run()
    _ = exec.Command("git", "-C", localPath, "commit", "-m", "Update").Run()

    // Push again with force (should succeed)
    // Note: Normal push might be rejected by git if branch has diverged
    // Using git push --force semantics via direct command
    cmd := exec.Command("git", "-C", localPath, "push", "--force", "origin", "feature/existing")
    if output, err := cmd.CombinedOutput(); err != nil {
        t.Fatalf("Second push (force) failed: %v, output: %s", err, string(output))
    }

    // Verify the branch is updated in remote
    output, err = exec.Command("git", "-C", remotePath, "log", "--oneline", "-1").CombinedOutput()
    if err != nil {
        t.Fatalf("failed to check remote log: %v", err)
    }

    if !strings.Contains(string(output), "Update") {
        t.Logf("remote branch log: %s", string(output))
        // Check if at least the branch exists
        branchOutput, _ := exec.Command("git", "-C", remotePath, "branch", "-a").CombinedOutput()
        t.Logf("remote branches: %s", string(branchOutput))
    }
}
